    ...
    */  
    ...
    public function newpass() : array {
        try {
            ...
            $pass = $http->request->get('pass');
            $newpass = $http->request->get('newpass');
            $newpass2 = $http->request->get('newpass2');
            # Comprobar si las nuevas contrañes son iguales
            if ($newpass !== $newpass2) {
                throw new ModelsException('Las nuevas contraseñas no coinciden');
                
            }

            # Comprobar si la contraseña original existe en la base de datos
            $query = $this->db->select('id_user,pass','users');
            if (Strings::chash($query[0]['pass'],$pass) == false){
                throw new ModelsException('Contraseña incorrecta.');
            }

            if (false !== $query && Strings::chash($query[0]['pass'],$pass)){
                $this->db->update('users',array(
                    'pass' => Strings::hash($newpass)
                ),"'id_user'='$this->id'",'LIMIT 1');

                return array('success' => 1, 'message' => 'Contraseña cambiada.');
            }
        } catch(ModelsException $e) {
            return array('success' => 0, 'message' => $e->getMessage());
        }
